...
export function PreviewSheet({ article, onClose }: PreviewSheetProps) {
  ...
  const canEmbed = data?.canEmbed ?? false
  ...
  return (
    <Sheet open={!!article} onOpenChange={(open) => { if (!open) onClose() }}>
      ...
      >
        <SheetTitle className="sr-only">{article?.title ?? "Article preview"}</SheetTitle>
          ...
          <div className="flex items-center gap-0.5 rounded-full border border-white/10 bg-white/5 p-0.5">
            <button
              ...
              className={cn(
                ...
                mode === "reader" ? "bg-white text-black" : "text-zinc-400 hover:text-white"
              )}
            >
            ...
            <button
              onClick={() => canEmbed && setMode("live")}
              disabled={!canEmbed}
              title={canEmbed ? "View the live page" : "This site can't be embedded"}
              className={cn(
                "flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-medium transition-colors",
                mode === "live" ? "bg-white text-black" : "text-zinc-400 hover:text-white",
                !canEmbed && "cursor-not-allowed opacity-40 hover:text-zinc-400"
              )}
            >
        ...
        <div className="min-h-0 flex-1">
          {loading ? (
            ...
            >
              <div className={cn("mx-auto", readerWidth === "wide" ? "max-w-4xl" : "max-w-2xl")}>
                {article?.feedName && (
                  <p
                ...
                {data?.readerHtml ? (
                  ...
                  */
                  <>
                    {article?.description && (
                      <p className="text-base leading-relaxed" style={{ color: "var(--reader-text)" }}>
                        {article.description.replace(/<[^>]*>/g, "").replace(/\s+/g, " ").trim()}
                      </p>
                    )}
                    <p className="mt-6 text-sm italic" style={{ color: "var(--reader-muted)" }}>
                      Sign in to Sparkfeed to read the full article here.
                    </p>
                  </>
                ) : (
                  <p className="italic" style={{ color: "var(--reader-muted)" }}>
                ...
                <div className="mt-12 flex justify-center pb-12">
                  <a
                    ...
                    className="h-10 rounded-full border px-8 text-sm font-bold leading-10 transition-opacity hover:opacity-80"
                    style={{
