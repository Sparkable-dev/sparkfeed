...
export function RouteError({ error, reset }: { error: unknown; reset?: () => void }) {
  ...
  return (
    <div className="flex min-h-[60vh] w-full items-center justify-center p-6">
      <div className="flex w-full max-w-md flex-col items-center gap-4 rounded-lg border border-white/10 bg-white/[0.02] p-8 text-center">
        <div className="rounded-full border border-amber-500/20 bg-amber-500/10 p-3">
          <AlertTriangle className="h-5 w-5 text-amber-400" />
        </div>

        <div className="flex flex-col gap-1.5">
          <h2 className="text-sm font-semibold text-white">This page could not load</h2>
        ...
        {detail && (
          <pre className="max-h-32 w-full overflow-auto rounded border border-white/10 bg-black/40 p-2 text-left text-[10px] text-zinc-500">
            {detail}
          </pre>
        )}
