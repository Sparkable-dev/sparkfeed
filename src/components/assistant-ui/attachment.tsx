...
const useAttachmentSrc = () => {
  const { file, src } = useAuiState(
    useShallow((s): { file?: File; src?: string } => {
      ...
      const attachmentSrc = s.attachment.content?.filter((c) => c.type === "image")[0]
        ?.image
      if (!attachmentSrc) return {}
      return { src: attachmentSrc }
    })
  )
...
const AttachmentThumb: FC = () => {
  ...
  return (
    <Avatar className="aui-attachment-tile-avatar h-full w-full rounded-none after:hidden">
      ...
      />
      <AvatarFallback>
        <FileText className="aui-attachment-tile-fallback-icon size-6 stroke-[1.5] text-muted-foreground/80" />
      </AvatarFallback>
    </Avatar>
  )
}
    ...
    <ComposerPrimitive.AddAttachment
      render={
        <TooltipIconButton
          ...
          variant="ghost"
          size="icon"
          className="aui-composer-add-attachment size-7 rounded-full p-1 text-xs font-semibold hover:bg-muted-foreground/15 active:scale-[0.96] motion-reduce:transition-none dark:border-muted-foreground/15 dark:hover:bg-muted-foreground/30"
