...
function SidebarInset({ className, ...props }: React.ComponentProps<"main">) {
  return (
    <main
      ...
      className={cn(
        // min-w-0: as a flex child this defaults to min-width:auto, so it is
        // sized by its widest descendant rather than by the space left over
        // beside the sidebar. Any horizontally scrolling content inside then
        // widens the whole page instead of scrolling within it. A no-op for
        // content that already fits, which is everything else in the app.
...
function SidebarContent({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "no-scrollbar flex min-h-0 flex-1 flex-col gap-0 overflow-auto group-data-[collapsible=icon]:items-center group-data-[collapsible=icon]:overflow-hidden",
        className
      )}
      {...props}
    />
