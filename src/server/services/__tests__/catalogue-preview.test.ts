...
const fetchPagePreview = vi.hoisted(() => vi.fn())
vi.mock("@/server/utils/fetch-page-articles", () => ({ fetchPagePreview }))
...
vi.mock("../catalogue", () => ({
  ensureCatalogueSynced: () => Promise.resolve(),
}))

const { getCataloguePreview, __resetPreviewState } =
  await import("../catalogue-preview")
...
async function freshDb(): Promise<Database> {
  ...
  await raw.execute(`CREATE TABLE catalogue_feeds (source_kind TEXT NOT NULL DEFAULT 'rss',
...
beforeEach(() => {
  ...
  fetchPagePreview.mockReset()
})
afterEach(() => vi.useRealTimers())

describe("catalogue preview", () => {
  it("previews website sources through page extraction instead of the feed parser", async () => {
    const db = await freshDb()
    await addFeed(db, "page", { sourceKind: "page" })
    fetchPagePreview.mockResolvedValue([
      {
    ...
    const result = await getCataloguePreview("feed", "page", db)
    expect(result.feeds[0].articles[0].title).toBe("Website article")
    expect(fetchPagePreview).toHaveBeenCalledOnce()
    expect(fetchFeedItems).not.toHaveBeenCalled()
  })
  it("retries a failed preview after five minutes rather than waiting six hours", async () => {
    const db = await freshDb()
    await addFeed(db, "retry")
    fetchFeedItems
      .mockRejectedValueOnce(new Error("Failed to fetch feed: 403"))
      .mockResolvedValueOnce([item("https://example.com/article")])
    await getCataloguePreview("feed", "retry", db)
    await db
      .update(catalogueFeeds)
      .set({
        articlesFetchedAt: new Date(Date.now() - 6 * 60_000).toISOString(),
      })
      .where(eq(catalogueFeeds.slug, "retry"))
    const result = await getCataloguePreview("feed", "retry", db)
    expect(result.feeds[0].state).toBe("ok")
