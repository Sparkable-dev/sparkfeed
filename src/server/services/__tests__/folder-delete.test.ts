...
async function seedFolder(
  ...
  await db.insert(feeds).values({
    id: `${id}-page`,
    name: "p",
    url: `https://${id}-page.example/blog`,
    kind: "page",
    folderId: id,
    workspaceId: WS,
  })
...
async function countAll(db: Database) {
  return {
    ...
    folderShares: (
      await db.select({ id: folderShares.folderId }).from(folderShares)
    ).length,
    feedShares: (await db.select({ id: feedShares.feedId }).from(feedShares))
      .length,
    watchedPages: (
      await db
        .select({ id: feeds.id })
        .from(feeds)
        .where(eq(feeds.kind, "page"))
    ).length,
  }
}
...
describe("deleteFolderTree", () => {
  ...
  it("lets the same site be watched again afterwards", async () => {
    ...
    await expect(
      db.insert(feeds).values({
        id: "again",
        name: "p",
        url: "https://root-page.example/blog",
        kind: "page",
        folderId: null,
        workspaceId: WS,
      })
  ...
  it("does not follow parent_id into another workspace", async () => {
    ...
    await db
      .update(folders)
      .set({ workspaceId: "workspace-b" })
      .where(eq(folders.id, "foreign"))
  ...
  it("deletes children before their parents", async () => {
    ...
    expect(await collectFolderSubtree(db, "root", WS)).toEqual([
      "child",
      "root",
    ])
  })
