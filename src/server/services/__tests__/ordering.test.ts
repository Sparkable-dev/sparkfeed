...
async function freshDb(): Promise<Database> {
  ...
  const raw = (handle as unknown as { $client: ReturnType<typeof createClient> })
    .$client
...
describe('ordering', () => {
  it('sorts positioned folders first and unpositioned ones by creation', async () => {
    ...
    await db.insert(folders).values([
      { id: 'never-dragged-new', name: 'd', position: null, createdAt: '2026-02-01' },
      { id: 'second', name: 'b', position: 1, createdAt: '2026-01-02' },
      { id: 'never-dragged-old', name: 'c', position: null, createdAt: '2026-01-01' },
  ...
  it('orders feeds the same way', async () => {
    ...
    await db.insert(feeds).values([
      { id: 'untouched', name: 'u', url: 'https://u.example', position: null, createdAt: '2026-01-05' },
      { id: 'dragged', name: 'd', url: 'https://d.example', position: 0, createdAt: '2026-09-09' },
    ])

    const rows = await db.select({ id: feeds.id }).from(feeds).orderBy(...FEED_ORDER)
