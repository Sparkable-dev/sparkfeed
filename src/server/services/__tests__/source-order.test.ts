...
async function freshDb(): Promise<Database> {
  const handle = createDb(`file:${join(workdir, `t${dbCount++}.db`)}`, { sqlite: true })
...
async function seed(db: Database) {
  await db.insert(folders).values([
    ...
    { id: 'foreign', name: 'Theirs', workspaceId: OTHER, createdAt: '2026-01-01' },
  ])
  await db.insert(feeds).values([
    { id: 'a', name: 'A', url: 'https://a.example', folderId: 'f1', workspaceId: WS, createdAt: '2026-01-01' },
    { id: 'b', name: 'B', url: 'https://b.example', folderId: 'f1', workspaceId: WS, createdAt: '2026-01-02' },
    { id: 'c', name: 'C', url: 'https://c.example', folderId: null, workspaceId: WS, createdAt: '2026-01-03' },
  ])
}

const readFolders = (db: Database) =>
  db.select({ id: folders.id }).from(folders).orderBy(...FOLDER_ORDER)

const readFeeds = (db: Database) =>
  db
    .select({ id: feeds.id, folderId: feeds.folderId, position: feeds.position })
    .from(feeds)
    .orderBy(...FEED_ORDER)
...
describe('applySourceOrder', () => {
  it('writes folder order, and the read path reflects it', async () => {
    ...
    expect((await readFolders(db)).map((f) => f.id)).toEqual(['f2', 'f1', 'foreign'])
  })
  ...
  it("refuses another workspace's folder in the order list", async () => {
    ...
    expect(await applySourceOrder(db, WS, { folders: ['f1', 'foreign'], feeds: [] })).toEqual({
      status: 'stale',
    })
  })
  ...
  it('rolls back the whole arrangement if a write fails partway', async () => {
    ...
    */
    ...
    expect((await readFolders(db)).map((f) => f.id)).toEqual(['f1', 'f2', 'foreign'])
  })
})
