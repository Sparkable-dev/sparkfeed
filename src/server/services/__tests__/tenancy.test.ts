...
async function freshDb(): Promise<Database> {
  ...
  await handle.insert(feeds).values([
    { id: 'feed-a', name: 'A feed', url: 'https://a.example/rss', workspaceId: WS_A },
    { id: 'feed-b', name: 'B feed', url: 'https://b.example/rss', workspaceId: WS_B },
    { id: 'feed-legacy', name: 'Legacy feed', url: 'https://l.example/rss', workspaceId: null },
  ])
  await handle.insert(articles).values([
    { id: 'art-a', feedId: 'feed-a', title: 'A article', link: 'https://a.example/1' },
    { id: 'art-b', feedId: 'feed-b', title: 'B article', link: 'https://b.example/1' },
    { id: 'art-legacy', feedId: 'feed-legacy', title: 'Legacy', link: 'https://l.example/1' },
    // Orphan: no feed row, so it belongs to no workspace and must never match.
    { id: 'art-orphan', feedId: 'feed-gone', title: 'Orphan', link: 'https://x.example/1' },
  ])
  return handle
}
...
describe('workspace predicates', () => {
  it('folderInWorkspace matches only that workspace', async () => {
    ...
    const rows = await db.select({ id: folders.id }).from(folders).where(folderInWorkspace(WS_A))
    expect(rows.map(r => r.id)).toEqual(['fol-a'])
  })

  it('feedInWorkspace matches only that workspace', async () => {
    const db = await freshDb()
    const rows = await db.select({ id: feeds.id }).from(feeds).where(feedInWorkspace(WS_B))
  ...
  it('a null workspace scopes to legacy rows, it does not disable the filter', async () => {
    ...
    const db = await freshDb()
    const rows = await db.select({ id: feeds.id }).from(feeds).where(feedInWorkspace(null))
    expect(rows.map(r => r.id)).toEqual(['feed-legacy'])
  })

  it('articleInWorkspace reaches ownership through the feed join', async () => {
    const db = await freshDb()
    const rows = await db.select({ id: articles.id }).from(articles).where(articleInWorkspace(WS_A))
