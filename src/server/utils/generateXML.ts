...
import { articles as articlesTable, feeds as feedsTable, folders as foldersTable } from "@/db/schema"
...
export async function generateXMLFeed(rawFolderSlug: string, origin: string, rawFeedSlug?: string) {
  ...
  */
  ...
  // Fetch RSS articles
  const rssArticles = targetFeedIds.length > 0
  ...
  const rssItems = rssArticles.map((article) => ({
    ...
    pubDate: article.publishedAt ? new Date(article.publishedAt).toUTCString() : '',
    guid: article.id,
    image: article.image || null,
  }))


  const allItems = [...rssItems].sort(
